...
// Generate simple token (this is a simplified version for client-side)
function generateFormToken() {
    const formTs = document.getElementById('form_ts').value;
    const userAgent = navigator.userAgent || '';
    const path = window.location.pathname || '/';
    const base = userAgent + '|' + formTs + '|' + path;
    ...
    // Convert to base36 for shorter token
    return formTs + ':' + hash.toString(36);
}

// Handle form submission
contactForm.addEventListener('submit', function(e) {
    // Set the form token before submission
    const formToken = document.getElementById('form_token');
    if (formToken) {
        formToken.value = generateFormToken();
    }
    ...
    if (name && email) {
        alert(`Thank you, ${name}! Your message is being sent. I'll get back to you soon at ${email}.`);
    }
    
    // Form will submit normally to sendmail.php
    // Don't prevent default so the form actually submits
});
